...
export async function ambilLaporanTransaksi(filter = {}) {
  const transaksi = await ambilSemuaTransaksi(filter);
  if (transaksi.length === 0) return [];

  const ids = transaksi.map((t) => t.id_transaksi);
  const { rows: items } = await getPool().query(
    `SELECT id_item, id_transaksi, nama_produk, harga_satuan, jumlah, subtotal
     FROM transaksi_item
     WHERE id_transaksi = ANY($1::int[])
     ORDER BY id_transaksi DESC, id_item ASC`,
    [ids]
  );

  const itemMap = new Map();
  for (const item of items) {
    if (!itemMap.has(item.id_transaksi)) itemMap.set(item.id_transaksi, []);
    itemMap.get(item.id_transaksi).push(item);
  }

  return transaksi.map((t) => ({
    ...t,
    items: itemMap.get(t.id_transaksi) || [],
  }));
}
